...
if ($method === 'POST') {
    ...
    // Validar duplicidade ou se a mesa já está reservada por outro participante
    ...
    foreach ($mesas_selecionadas as $num_mesa) {
        $items[] = [
            "title" => "Reserva de Mesa #" . $num_mesa . " - Evento Beneficente",
            "quantity" => 1,
            "currency_id" => "BRL",
    ...
    $mp_payload = [
        ...
        "back_urls" => [
            "success" => $base_url . "/reserva_mesas.php?status=success&ref=" . $external_reference,
            "failure" => $base_url . "/reserva_mesas.php?status=failure",
            "pending" => $base_url . "/reserva_mesas.php?status=pending"
        ],
    ...
    if ($http_code === 200 || $http_code === 201) {
        ...
        // Salvar a preferência do MP nas reservas
